...
import { Plus, Edit2, Trash2, Check, X, Package, Layers, Calculator, PackagePlus } from 'lucide-react';
import { quotationTemplatesService, QuotationTemplateChapter, QuotationTemplateLine } from '../../../services/quotation-templates.service';
import { Resource, ResourceType } from '../../../services/resources.service';
...
interface TemplateChapterEditorProps {
  templateId: string;
  chapter: QuotationTemplateChapter;
  index: number;
  onUpdate: () => void;
}
...
export default function TemplateChapterEditor({ templateId, chapter, index, onUpdate }: TemplateChapterEditorProps) {
  ...
  const [lineForm, setLineForm] = useState<Partial<QuotationTemplateLine> & { title?: string, description?: string }>({});
  const [editingApuLine, setEditingApuLine] = useState<QuotationTemplateLine | null>(null);
  ...
  const saveChapterTitle = async () => {
    try {
      await quotationTemplatesService.updateChapter(chapter.id, { 
        title: chapterTitle
      });
  ...
  const deleteChapter = async () => {
    ...
    try {
      await quotationTemplatesService.deleteChapter(chapter.id);
      onUpdate();
  ...
  // --- Line Actions ---
  const startEditLine = (line: QuotationTemplateLine, autoCode?: string) => {
  ...
  const saveLine = async () => {
    try {
      ...
      if (isAddingLine) {
        await quotationTemplatesService.addLine(chapter.id, {
          concept: fullConcept,
          ...
          isApu: lineForm.isApu || false
        });
      } else if (editingLineId) {
        await quotationTemplatesService.updateLine(editingLineId, {
          concept: fullConcept,
          unit: lineForm.unit,
  ...
  const deleteLine = async (lineId: string) => {
    ...
    try {
      await quotationTemplatesService.deleteLine(lineId);
      onUpdate();
    } catch (e) {
      toast.error('Error al eliminar');
    }
  ...
  const handleSelectResource = (resource: Resource) => {
    const [title, ...descParts] = (resource.name || '').split('\n');
    setLineForm(prev => ({
      ...prev,
      title,
      description: descParts.join('\n'),
      unit: resource.unit,
      unitPrice: resource.salesPrice || resource.unitCost,
      quantity: (resource as any).defaultQuantity ?? 1,
      resourceId: resource.id,
      isApu: resource.resourceType === ResourceType.APU
    }));
    ...
    <div className="border border-slate-200 bg-white rounded-xl overflow-hidden shadow-sm mb-4 transition-all hover:border-slate-300">
      ...
      <table className="w-full text-sm">
        ...
        <tbody className="divide-y divide-slate-100">
                    ...
                    <>
                      ...
                      <td className="py-3 px-4 cursor-text" onClick={() => startEditLine(l, `${index + 1}.${lIdx + 1}`)}>
                        <div className="flex items-start gap-2">
                          <button 
                            onClick={(e) => { e.stopPropagation(); quotationTemplatesService.updateLine(l.id, { isGroup: !isGroup }).then(onUpdate); }}
                          ...
                          {!isGroup && (
                            isApu ? (
                              <div className="flex items-center gap-0.5">
                                ...
                                <button 
                                  onClick={(e) => { e.stopPropagation(); quotationTemplatesService.updateLine(l.id, { isApu: false }).then(onUpdate); }}
                              ...
                              <button 
                                onClick={(e) => { e.stopPropagation(); quotationTemplatesService.updateLine(l.id, { isApu: true, isGroup: false }).then(onUpdate); }}
                            ...
                            <div className="flex items-start gap-2">
                              {cl.isApu ? (
                                <div className="flex items-center gap-0.5">
                                  ...
                                  <button 
                                    onClick={(e) => { e.stopPropagation(); quotationTemplatesService.updateLine(cl.id, { isApu: false }).then(onUpdate); }}
                                    className="mt-0.5 p-1 rounded transition-colors text-slate-300 hover:text-red-500 hover:bg-slate-100"
                                    title="Convertir en partida manual (desvincular APU)"
                                  >
                                    <X size={10} />
                                ...
                                <button 
                                  onClick={(e) => { e.stopPropagation(); quotationTemplatesService.updateLine(cl.id, { isApu: true, isGroup: false }).then(onUpdate); }}
          ...
          {!isAddingLine && (!chapter.lines || chapter.lines.filter(l => !l.parentId).length === 0) && (
            <tr><td colSpan={7} className="py-6 text-center text-slate-400">No hay partidas en este capítulo de plantilla</td></tr>
          )}
        </tbody>
      </table>
      ...
      <div className="bg-slate-50 px-4 py-3 border-t border-slate-200 flex justify-between items-center">
        ...
        <div className="text-sm font-medium text-slate-500 flex items-center gap-3">
          <span>{chapter.lines?.filter(l => !l.parentId).length || 0} Partidas</span>
          ...
          <span>SUBTOTAL CAPÍTULO: <span className="font-bold text-slate-800 ml-1 text-base">
            {formatCurrency(chapter.lines?.filter(l => !l.parentId).reduce((a, l) => { 
              const children = chapter.lines.filter(cl => cl.parentId === l.id); 
              const baseCost = (l.isGroup || l.isApu) ? children.reduce((sum, cl) => sum + (cl.quantity * cl.unitPrice), 0) : l.unitPrice;
              const computedUnitPrice = l.isApu ? baseCost * (1 + (l.margin || 0) / 100) : baseCost;
              return a + (l.quantity * computedUnitPrice); 
            }, 0) || 0)}
          </span></span>
        </div>
      </div>
