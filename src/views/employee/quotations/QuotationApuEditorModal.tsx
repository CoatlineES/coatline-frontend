...
import { QuotationLine } from '../../../types/quotation';
...
interface QuotationApuEditorModalProps {
  ...
  chapterId: string;
  line: QuotationLine;
  childrenLines: QuotationLine[];
  onClose: () => void;
  onUpdate: () => void;
}

export default function QuotationApuEditorModal({ 
  quotationId, 
  chapterId, 
  line, 
  childrenLines, 
  ...
  // Sin embargo, para que el `unitPrice` de la línea Padre (el APU) se actualice, 
  ...
  const handleSaveAllAndClose = async () => {
    try {
      if (localMargin !== (line.margin || 0)) {
        await quotationsService.updateLine(quotationId, chapterId, line.id, { margin: localMargin });
        onUpdate();
      }
      onClose();
    } catch (error) {
  ...
  const handleUpdateChild = async (childId: string, field: string, value: any) => {
    try {
      await quotationsService.updateLine(quotationId, chapterId, childId, { [field]: value });
      updateParentCost();
    } catch (error) {
  ...
  const handleDeleteChild = async (childId: string) => {
    ...
    try {
      await quotationsService.deleteLine(quotationId, chapterId, childId);
      updateParentCost();
    } catch (error) {
  ...
  const handleAddComponentFromLibrary = async (resource: Resource) => {
    ...
    try {
      await quotationsService.addLine(quotationId, chapterId, {
        parentId: line.id,
        ...
        <div className="bg-slate-50 px-6 py-4 border-b border-slate-200 flex justify-between items-start">
          <div>
            <div className="flex items-center gap-2 text-primary mb-1">
              <Package size={20} />
              <h3 className="font-bold text-lg leading-none">Desglose de APU</h3>
            </div>
        ...
        <div className="p-6 overflow-y-auto flex-1 bg-white">
          ...
          {childrenLines.length === 0 && !isAddingManual && (
            <div className="text-center py-8 text-slate-400 text-sm border-2 border-dashed border-slate-100 rounded-lg">
              Este APU en la cotización no tiene desglose. Añade componentes para calcular su coste.
            </div>
          )}
